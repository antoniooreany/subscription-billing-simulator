...
type RetryRepository interface {
	Create(ctx context.Context, ra model.RetryAttempt) (model.RetryAttempt, error)
	CountBySubscriptionID(ctx context.Context, subscriptionID string) (int, error)
	GetByIdempotencyKey(ctx context.Context, key string) (model.RetryAttempt, error)
}
...
func NewRetryService(retries RetryRepository, subscriptions SubscriptionRepository, events EventRepository) *RetryService {
	return &RetryService{
		retries:       retries,
...
func (s *RetryService) Run(ctx context.Context, subscriptionID, key string) (model.RetryAttempt, error) {
	...
	attempt := model.RetryAttempt{
		ID:             shared.NewID("retry"),
		SubscriptionID: subscriptionID,
		AttemptNumber:  count + 1,
		Status:         model.RetryProcessed,
		IdempotencyKey: key,
	}
